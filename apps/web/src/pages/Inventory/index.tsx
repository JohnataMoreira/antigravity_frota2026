...
export function InventoryList() {
    ...
    const moveMutation = useMutation({
        mutationFn: (data: any) => api.post(`/inventory/${selectedItem.id}/movements`, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['inventory'] });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <GlassCard className="flex items-center gap-4 py-6 border-amber-500/20">
                ...
                <div className="flex flex-col md:flex-row gap-4 items-center">
                    <div className="relative flex-1 group">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border-none focus:ring-2 focus:ring-primary/50 outline-none transition-all font-medium"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                        />
                    </div>
                    <div className="flex gap-2 p-1 bg-gray-100 dark:bg-gray-800/50 rounded-xl overflow-x-auto max-w-full no-scrollbar">
                        {categories.map(cat => (
                            <button
                                key={cat}
                                onClick={() => setCategoryFilter(cat)}
                                className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap ${categoryFilter === cat ? 'bg-white dark:bg-gray-700 shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'}`}
                            >
                                {cat === 'ALL' ? 'Todos' : cat}
                            </button>
                        ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredInventory.map((item: any) => (
                    <GlassCard key={item.id} className="group hover:shadow-xl transition-all duration-300 flex flex-col h-full border-gray-100 dark:border-gray-800/50">
                        ...
                        <div className="flex-1">
                            <h3 className="text-xl font-black text-gray-900 dark:text-white leading-tight mb-1">{item.name}</h3>
                            <p className="text-xs font-bold text-muted-foreground uppercase mb-4 tracking-widest">SKU: {item.sku || 'N/A'}</p>

                            <div className="grid grid-cols-2 gap-4 mb-6">
                                <div className="p-3 bg-gray-50 dark:bg-gray-800/30 rounded-2xl">
                            ...
                            {item.currentQuantity <= item.minQuantity && (
                                <div className="flex items-center gap-2 px-3 py-2 bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-xl mb-4 text-xs font-bold animate-pulse">
                        ...
                        <div className="flex gap-2 mt-2">
                            ...
                            <button title="Histórico" className="p-3 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl transition-all">
                                <History size={18} />
                            </button>
                        </div>
                    </GlassCard>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <GlassCard className="w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setIsMoveModalOpen(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition-colors">
                                <X size={20} />
                            </button>
                        </div>

                        <div className="p-4 bg-gray-50 dark:bg-gray-800/30 rounded-2xl mb-6">
                            <p className="text-[10px] font-black text-muted-foreground uppercase mb-1 opacity-50">Item Selecionado</p>
                            <p className="font-bold text-gray-900 dark:text-gray-100">{selectedItem?.name}</p>
                        ...
                        <form onSubmit={(e) => {
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input name="quantity" type="number" step="0.01" required autoFocus className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl outline-none focus:ring-2 focus:ring-primary/50 font-medium" placeholder="0.00" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase text-muted-foreground mb-1 block tracking-widest">Motivo</label>
                                    <select name="reason" required className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl outline-none focus:ring-2 focus:ring-primary/50 font-medium">
                            ...
                            <div>
                                ...
                                <textarea name="notes" rows={3} className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-xl outline-none focus:ring-2 focus:ring-primary/50 font-medium resize-none" placeholder="Opcional. Ex: Nota fiscal, veículo utilizado, etc."></textarea>
                            </div>
                            <Button
                                type="submit"
