                ...
                {users.map((user: any) => (
                    <GlassCard key={user.id} className={`group transition-all hover:scale-[1.02] border-t-4 ${user.active ? 'border-blue-500' : 'border-gray-400 opacity-60'}`}>
                        <div className="flex justify-between items-start mb-4">
                            <div className={`p-3 rounded-2xl ${user.role === 'ADMIN' ? 'bg-purple-100 text-purple-600 dark:bg-purple-900/30' : 'bg-blue-100 text-blue-600 dark:bg-blue-900/30'}`}>
                                {user.role === 'ADMIN' ? <Shield size={24} /> : <Truck size={24} />}
                            </div>
                            <div className="flex gap-2">
                                <button
                                    onClick={() => handleEdit(user)}
                                    className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors text-gray-400 hover:text-blue-500"
                                >
                                    <Edit2 size={18} />
                                </button>
                                <button
                                    ...
                                    className={`p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors ${user.active ? 'text-green-500' : 'text-red-500'}`}
