...
export function PurchaseOrderDetailsModal({ order, isOpen, onClose }: PurchaseOrderDetailsModalProps) {
    ...
    const mutation = useMutation({
        mutationFn: ({ action, data }: { action: string, data?: any }) =>
            api.post(`/purchasing/orders/${order.id}/${action}`, data),
        onSuccess: () => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white dark:bg-gray-950 w-full max-w-2xl rounded-3xl shadow-2xl border border-white/10 overflow-hidden flex flex-col max-h-[90vh]">
                <header className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/50">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        {order.items.map((item: any) => (
                            <div key={item.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800">
                                <div>
                    ...
                    {/* Admin Actions */}
                    {canApprove && (
                        <div className="p-6 bg-blue-50 dark:bg-blue-900/20 rounded-3xl border border-blue-100 dark:border-blue-900/30 space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        type="number"
                                        placeholder="0,00"
                                        className="w-full px-4 py-3 bg-white dark:bg-gray-950 border border-blue-200 dark:border-blue-800 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/40 font-bold"
                                        value={totalValue}
                                ...
                                <div className="space-y-1">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full px-4 py-3 bg-white dark:bg-gray-950 border border-blue-200 dark:border-blue-800 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/40 text-sm"
                                        value={approveNotes}
                ...
                <footer className="p-6 border-t border-gray-100 dark:border-gray-800 flex flex-wrap gap-3 justify-end">
                        ...
                        <button
                            onClick={() => {
                                if (confirm('Deseja cancelar este pedido?')) {
                                    api.patch(`/purchasing/orders/${order.id}/cancel`).then(() => {
                                        queryClient.invalidateQueries({ queryKey: ['purchase-orders'] });
                                        onClose();
                                    });
                                }
