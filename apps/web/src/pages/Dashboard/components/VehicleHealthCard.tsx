    ...
    return (
        <GlassCard className="h-full flex flex-col">
            <div className="flex justify-between items-start mb-6">
                <div>
                    <h3 className="text-xl font-black text-gray-900 dark:text-white">Saúde da Frota</h3>
                    <p className="text-xs text-muted-foreground font-bold uppercase tracking-wider">Manutenção Preventiva</p>
                </div>
                <div className="flex gap-2">
                    {criticalCount > 0 && (
                        <div className="px-3 py-1 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-[10px] font-black rounded-full animate-pulse">
                            {criticalCount} CRÍTICO
                        </div>
                    )}
                    {warningCount > 0 && (
                        <div className="px-3 py-1 bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 text-[10px] font-black rounded-full">
                            {warningCount} ALERTA
                        </div>
                    )}
                </div>
                ...
                {alerts.slice(0, 4).map((alert) => (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-xl ${alert.severity === 'CRITICAL' ? 'bg-red-100 dark:bg-red-900/30 text-red-600' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-600'}`}>
                                <Wrench size={16} />
                            </div>
                            <div>
                                <div className="font-bold text-sm text-gray-900 dark:text-white">{alert.plate}</div>
            ...
            <button
                onClick={() => navigate('/maintenance')}
                className="w-full mt-6 py-3 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl text-xs font-black text-muted-foreground hover:bg-gray-50 dark:hover:bg-gray-800/50 hover:text-primary transition-all flex items-center justify-center gap-2 group"
            >
