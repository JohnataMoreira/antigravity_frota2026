...
export function ActiveJourneysWidget({ journeys, isLoading }: ActiveJourneysWidgetProps) {
    if (isLoading) {
        return (
            <div className="bg-card/50 backdrop-blur-md border border-white/10 rounded-3xl p-6 shadow-xl animate-pulse">
                <div className="h-6 w-48 bg-muted rounded-lg mb-6"></div>
                <div className="space-y-4">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="h-16 w-full bg-muted/50 rounded-2xl"></div>
                    ))}
                </div>
            </div>
        );
    ...
    return (
        <div className="bg-card/50 backdrop-blur-md border border-white/10 rounded-3xl p-6 shadow-xl flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-black uppercase tracking-tight flex items-center gap-2">
                    <Activity className="w-5 h-5 text-emerald-500" />
                    Em Rota Agora
                </h2>
                <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 text-[10px] font-black rounded-full uppercase border border-emerald-500/20">
                    ...
                    journeys.map((journey) => (
                        <div
                            ...
                            className={`group p-4 rounded-2xl border transition-all hover:shadow-lg hover:scale-[1.02] cursor-pointer ${journey.isDeviated
                                ...
                                : 'bg-muted/30 border-white/5 hover:bg-muted/60'
                                }`}
                        >
            ...
            {journeys.length > 0 && (
                <button className="mt-4 w-full py-3 bg-muted/50 hover:bg-muted text-[10px] font-black uppercase tracking-widest text-muted-foreground rounded-xl transition-all flex items-center justify-center gap-2 group border border-white/5">
                    Ver Mapa Completo
